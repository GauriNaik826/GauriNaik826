<h1 align="center">Hi 👋, I'm Gauri Naik</h1>
<h3 align="center">Machine Learning Engineer | LLM Researcher | AI Systems Builder</h3>

---

🎓 MS CS @ USC  
📚 Published at ACL 2024  
🤖 Working on LLMs, RAG Systems, Vision-Language-Action Models & MLOps  
📍 Los Angeles, CA  

---

## 🧠 About Me

I build scalable AI systems that combine research rigor with production engineering.

My work spans:
- Large Language Models (fine-tuning, prefix tuning, prompt optimization)
- Agentic RAG pipelines
- Vision-Language-Action models for robotics
- Faithfulness & hallucination reduction in LLMs
- MLOps & reproducible ML infrastructure

I enjoy bridging **research → production AI systems**.

---

## 🔬 Research Experience

### 🧪 Graduate Research Assistant — USC Viterbi (CPS-Vida Lab)
### 📄 ACL 2024 — Perspective-aware Healthcare Summarization
🔗 Paper: [Link]  
🔗 Code: [Link]

---

## 🚀 Selected Projects

### Agentic LLM Twin with Advanced RAG - 🔗 [View Repository](https://github.com/GauriNaik826/Agentic_LLM_Twin_RAG)
### IMDB Sentiment MLOps Pipeline - 🔗 [View Repository](https://github.com/GauriNaik826/MLops_end_to_end)
### Book Reccomender system - 🔗 [View Repository](https://github.com/GauriNaik826/Book-Recommender-System)
### n8n_Daily_Weather_Email_Automation - 🔗 [View Repository](https://github.com/GauriNaik826/n8n_Daily_Weather_Automation)

---

## 🛠 Tech Stack

**Languages:** Python, C++, SQL  
**ML:** PyTorch, TensorFlow, HuggingFace, DGL  
**LLMs:** Fine-tuning, Prefix tuning, Prompt engineering, RAG  
**MLOps:** MLflow, DVC, Docker, AWS, CI/CD  
**Infra:** Kubernetes, SageMaker, SLURM  
**Databases:** PostgreSQL, MongoDB, MySQL  

---

## 📫 Connect With Me

LinkedIn: https://www.linkedin.com/in/gauri-naik-bb83a21a5/
Email: gmnaik@usc.edu  
Google Scholar: https://scholar.google.com/citations?hl=en&user=q2E_fm0AAAAJ


